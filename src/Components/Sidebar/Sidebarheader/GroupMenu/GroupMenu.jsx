...
export default function GroupMenu() {
    ...
    return (
        <div className={`mx-auto max-w-xs overflow-hidden border-x ${openWindow === dataArr.length - 1 ? 'border-x border-b' : 'border-x'} rounded-lg border-zinc-600`}>
            {dataArr?.map((data, idx) => {
                const { key, title, svg } = data;
                return (
                    <div key={key}>
                        <div onClick={() => {
                            setOpenWindow(true)
                            setOpenGroup(!openGroup)
                        }} className="relative flex cursor-pointer items-center gap-2 bg-zinc-600 p-4">
                            {svg}
